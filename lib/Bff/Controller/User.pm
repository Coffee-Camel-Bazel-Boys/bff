...
sub post_user ($user) {
    eval { return $client->post( $CLIENT_URI => json => $user )->result; } or do {
        return undef;
    }
}

sub put_user ( $user, $id ) {
    eval { return $client->put( $CLIENT_URI . $id => json => $user )->result; } or do {
        return undef;
    }
}

sub post_login ($login) {
    eval { return $client->post( $CLIENT_URI => json => $login )->result; } or do {
        return undef;
    }
}

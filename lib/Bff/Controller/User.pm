...
sub create {
  ...
  $self->render(json => $user_create_response->json, status => $user_create_response->code);
}
...
sub find {
  ...
  my $id = $self->stash('id');
  return $self->render(status => 403, json => {err => 403, msg => 'Unauthorized'})
    unless $self->session('user_id') eq $id;
...
sub login {
  ...
  $self->render(json => $user_login_response->json, status => $user_login_response->code);
}

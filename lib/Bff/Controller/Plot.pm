...
sub get_pictures_by_id {
    eval { return $client->get( PLOT_PICTURE_URI(shift) )->result; } or do {
        return undef;
    }
}
...
sub put_plot ( $plot, $id ) {
    eval { return $client->put( ( PLOT_URI() . $id ) => json => $plot )->result; } or do {
        return undef;
    }
}
...
sub find_plot {
    my $id = shift;
    eval { return $client->get( PLOT_URI() . $id )->result; } or do {
        return undef;
    }
}

sub find_all_plots {
    eval { return $client->get( PLOT_URI() )->result; } or do {
        return undef;
    }
}

sub delete_plot ($id) {
    eval { return $client->delete( PLOT_URI() . $id )->result; } or do {
        return undef;
    }
}
...
sub create ($self) {
    ...
    return $self->render(
        json   => $plot_create_response->json,
